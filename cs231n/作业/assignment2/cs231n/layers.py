...
def batchnorm_backward(dout, cache):
    ...
    dmean2 = dvar * -2.0 / N * np.sum(x - mean, axis =0, keepdims = True)
    dmean = dmean1 + dmean2
    
    dx1 = dx_norm * 1.0 / np.sqrt(var + eps)
    dx2 = dmean * 1.0 / N
    dx3 = dvar * 2.0 / N * (x - mean)
    dx = dx1 + dx2 + dx3
...
def batchnorm_backward_alt(dout, cache):
    ...
    dvar = np.sum(dx_norm * -1 / 2.0 * (x - mean) / (var + eps)**(3/2.0), axis = 0, keepdims = True)
    dmean1 = np.sum(dx_norm * -1.0 / np.sqrt(var + eps), axis = 0, keepdims = True)
    dmean2 = dvar * -2.0 / N * np.sum(x - mean, axis =0, keepdims = True)
    dmean = dmean1 + dmean2
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    N, C, HX, WX = x.shape    # x的输入格式
    F, C, HF, WF = w.shape    # 滤波器的格式
    pad = conv_param['pad']   # 填充个数
    stride = conv_param['stride']   # 步长
    assert(HX + 2 * pad - HF) % stride == 0   # 整除才通过
    assert(WX + 2 * pad - WF) % stride == 0  # 整除才通多
    H0 = int((HX + 2 * pad - HF) / stride + 1)   # 计算卷积后的图片高度
    W0 = int((WX + 2 * pad - WF) / stride + 1) # 计算卷积后的图片宽度
    
    out = np.zeros((N, F, H0, W0))    # 输出图片格式
    x_pad = np.pad(x, ((0,0), (0,0), (pad, pad), (pad, pad)), 'constant')  # 对输入图片进行边缘数据填充
    
    # 采用naive方法来进行卷积，即使用四个for 循环来进行
    for i in range(N):
        for f in range(F):
            for r in range(H0):
                for c in range(W0):
...
def conv_backward_naive(dout, cache):
    ...
    x, w, b, conv_param = cache  # 保存的本层的数据
    N, F, H0, W0 = dout.shape     # 上一层卷积的导数结果
    F, C, HF, WF = w.shape         # 卷积核的格式
    N, C, HX, WX = x.shape         # 输入数据的格式
    pad = conv_param['pad']       # 填充的个数
    stride = conv_param['stride']  # 步长
    
    dx = np.zeros_like(x)              # x的导数
    dw = np.zeros_like(w)            # w的导数
    db = np.zeros_like(b)             # b的导数
    x_pad = np.pad(x, ((0,0), (0,0), (pad, pad), (pad, pad)), 'constant')
    dx_pad = np.pad(dx, ((0,0), (0,0), (pad, pad), (pad, pad)), 'constant')
    db = np.sum(dout, axis = (0,2,3))   # b的导数
...
def max_pool_forward_naive(x, pool_param):
    ...
    N, C, HX, WX = x.shape
    HP = pool_param['pool_height']   # max_pooling的高度
    WP = pool_param['pool_width']   # max_pooling的宽度
    stride = pool_param['stride']       # max_pooling的步长
    H0 = int((HX - HP) / stride + 1)   # max_pooling后的高度
    W0 = int((WX - WP) / stride + 1) # max_pooling后的宽度
    out = np.zeros((N, C, H0, W0))
    #out_max_idx = np.zeros((N, C, H0, W0))
    # max_pooling的过程
    for i in range(N):
        for ch in range(C):
            for r in range(H0):
...
def max_pool_backward_naive(dout, cache):
    ...
    # np.argmax(array) 记住，这里返回的是数组array中最大值的序号，无论array是几维的，完全把它整
    # 合成一维，然后返回一维中最大值的序号
    x, pool_param = cache
    N, C, HX, WX = x.shape
    N, C, H0, W0 = dout.shape
    HP = pool_param['pool_height']   # 卷积的高度
    WP = pool_param['pool_width']   # 卷积的宽度
    stride = pool_param['stride']       # 卷积步长
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # 以前我么求batch_normal的时候，我们是对每一个维度进行均值和方差计算，而在进行卷积计算时，
    # 我们则要对每个通道进行进行均值和方差计算
    # 这里还要注意先要进行转置在进行维度转换
    N, C, H, W = x.shape
